...
const N8nConnectionForm: React.FC<N8nConnectionFormProps> = ({ userId, onSuccess }) => {
  ...
  if (fetching) return <div className="p-12"><LoadingSpinner size="medium" color="text-sky-500" /></div>;
    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-zinc-400 mb-2">N8N URL (Instância)</label>
          <input
            type="url"
            required
            ...
            className="w-full bg-zinc-950 border border-zinc-800 rounded-xl py-3 px-4 text-zinc-200 placeholder-zinc-700 focus:ring-2 focus:ring-sky-500/40 outline-none transition-all"
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-zinc-400 mb-2">Client ID (Webhook ID)</label>
          <input
            type="text"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-zinc-400 mb-2">Access Token / API Key</label>
          <input
            type="password"
            required
      ...
      <div className="flex items-start gap-3 p-4 bg-sky-500/5 border border-sky-500/10 rounded-xl group hover:border-sky-500/30 transition-colors">
        ...
        <p className="text-xs text-zinc-400 leading-relaxed">
          Segurança Ativada: Suas credenciais são protegidas com <span className="text-sky-400 font-bold uppercase">AES-256-CBC</span> e armazenadas como <span className="font-mono">encrypted_access_token</span>.
        </p>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4">
        <button
          type="submit"
          disabled={loading}
          className="bg-sky-600 hover:bg-sky-500 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-sky-600/20 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3"
        >
